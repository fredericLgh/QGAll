...
{
	QDalsaGige* pDlg = (QDalsaGige*)Info->GetContext();

	char strUserName[256] = { 0 };
	SapManager::GetResourceName(Info->GetServerIndex(), SapManager::ResourceAcqDevice, 0, strUserName);
	if (strUserName == pApp->m_CameraName)
	{
		if (Info->GetEventType() == SapManager::EventServerConnected)
		{
			/*pDlg->ReStartCamera();

			OutputDebugString(L"\n相机已经连接！");
			pDlg->m_TxtCameraStatus.SetBackgroundColor(GREEN);
			pDlg->m_TxtCameraStatus.Refresh();*/
		}

		if (Info->GetEventType() == SapManager::EventServerDisconnected)
		{
			/*OutputDebugString(L"\n相机断开连接！");
			pDlg->m_TxtCameraStatus.SetBackgroundColor(RED);
			pDlg->m_TxtCameraStatus.Refresh();*/
		}
...
void QCamWidget::OnClickedBtSave()
{
	...
	{
		m_PicID = 0;
		ui.m_pbRecord->setText(QString::fromLocal8Bit("停止"));
	}
	else
	{
		ui.m_lbCompressionTime->setText(QString::fromLocal8Bit(""));
		ui.m_pbRecord->setText(QString::fromLocal8Bit("录制"));
	}
